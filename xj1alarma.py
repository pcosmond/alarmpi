# LJ latest as from 23th April 2014 change test email section and remove printtoscreen reference and added logging and exception handling
...
user="simulatorlj@flighttrainingadelaide.com"     #change privateeyeuser to suit
...
EMAIL_FROM= "LJ XJ1 Simulator"       #change from to suit
EMAIL_SPACE=", "
EMAIL_SUBJECT="LJ XJ1 Simulator Alarm"      #Change subject to suit
DATE_FORMAT= "%d/%m/%Y"  
...
over_temp= "The XJ1 simulator room is over a safe Temperature limit"     #change messg to suit
power_failure= "The Power test  Simulator has failed"       #change messg to suit   
email_test= "Daily Email Test - Email Operational for XJ1"  #change messg to suit
...
def NotifyHostTemperature():
    ...
    tfile = open(filename,'a')
    text1 = tfile.write('Temp Buffer' + TempBuffer + '\n')
...
def NotifyHostGPIO(GPIOnumber):
    ...
    tfile = open(filename,'a')
    text1 = tfile.write('Notify Host GPIO Number' + rt + '\n')
    text1 = tfile.write('End of Temp Section' + '\n')
    tfile.close()
    return(rt)
...
def UpdateHost(function,opcode):
    ...
    try:
        ...
        tfile = open(filename,'a')
        text1 = tfile.write('Update Host' + rt + '\n')
        tfile.close()
