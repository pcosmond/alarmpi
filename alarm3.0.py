# LJ ver 3 latest as from 27th April 2014 Latest tested version update all sections
...
power_failure= "The Power has tripped"                           #change messg to suit   
email_test= "Daily Email Test - "                                #change messg to suit  
# logfile name
...
MaxTemp=32                                #change max temp to suit, trip temp is this temp minus 3 degrees.
...
def GetTemperatureEmail():    #Email sending triggers
    ...
    if input_value == 1: 
        ...
        tfile.close()
        SendEmailAlert(power_failure + "  Number 1 System")
    input_value2 = GPIO.input(15)
    if input_value2 == 1: 
        tfile = open(filename,'a')
    ...
    tfile.close()
    print 'Elapsed Email Delay   ' + str(elapsed_email_delay)
    print 'Elapsed Email Time-- Email 5 min delay   ' + str(elapsed_email_time)
    print 'Elapsed Host Delay   ' + str(elapsed_host_delay)
...
def GetTemperature():        # Privateeyepi triggers
    ...
    subprocess.call(['modprobe', 'w1-gpio'])
    subprocess.call(['modprobe', 'w1-therm'])
    filenamer = "/sys/bus/w1/devices/28-0000043df3d8/w1_slave"
